...
RUN curl -fsSL --connect-timeout 30 --max-time 120 --retry 3 --retry-delay 5 \
    "${GH_MIRROR}https://github.com/v2fly/domain-list-community/releases/download/20260717032527/dlc.dat" \
    -o /usr/local/bin/geosite.dat \
    && curl -fsSL --connect-timeout 30 --max-time 120 --retry 3 --retry-delay 5 \
    "${GH_MIRROR}https://github.com/v2fly/geoip/releases/download/202607171233/geoip.dat \

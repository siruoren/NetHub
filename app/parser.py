...
async def fetch_connected_proxies(async def fetch_connected_proxies(
        ...
        for sub_index, sub in enumerate(subscriptions):
            ...
            for conn_node in sub_connected:
                ...
                if best_match and best_quality >= 1:
                    ...
                    logger.debug("无法匹配节点: %s", conn_node)  # 增加日志记录无法匹配的节点
            ...
            for conn_node in unmatched:
                ...
                for link_info in share_links:
                    ...
                    port_match = (conn_node["port"] and link_info.port
                                  and str(conn_node["port"]) == str(link_info.port))  # 确保端口匹配
                    if addr_exact:
                        quality = 4
                ...
                if best_match and best_quality >= 1:
                    ...
                    logger.debug("无法匹配节点 (第二次匹配): %s", conn_node)  # 增加日志记录无法匹配的节点
